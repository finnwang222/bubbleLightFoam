...
:
    ...
    rho_(coeffs_.getOrDefault<scalar>("rho", 0)),
    dp_(coeffs_.getOrDefault<scalar>("dp", 0))
{
    ...
    forAll(kLambda_, iBand)
    {
        ...
        forAll(alpha_, cellI)
        {
            kLambda_[iBand][cellI] = (aBand_[iBand]+sBand_[iBand]*(1-alpha_[cellI]))+3/2*rho_*pow(alpha_[cellI],2)/dp_;
        } 
    }
    ...
    forAll(sLambda_, iBand)
    {
        ...
        forAll(alpha_, cellI)
        {
            sLambda_[iBand][cellI] = sBand_[iBand]*(1-alpha_[cellI])+3/2*rho_*pow(alpha_[cellI],2)/dp_;
        } 
    }
